...
echo "Please enter the location of the chroot directory (This will be refreshed fully) [q to exit] : "
read chroot_dir
if [ "$chroot_dir" == "q" ]; then
    echo "Exiting..."
    exit 0
fi
if [ -d "$chroot_dir" ]; then
    ...
    echo "Chroot not found... Setting this as the new Chroot."
    exit 0
fi
...
cp $backup_dir/etc/localtime ${CHROOT_DIR}/etc/
cp -a $backup_dir/usr/share/dns ${CHROOT_DIR}/usr/share/
